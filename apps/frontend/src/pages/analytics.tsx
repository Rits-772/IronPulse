...
const StrengthCurve = memo(({ data }: { data: any[] }) => (
...
const MuscleHeatmap = memo(({ data }: { data: any[] }) => (
  <div className="h-80 w-full flex items-center justify-center">
    <ResponsiveContainer width="100%" height="100%">
      <RadarChart cx="50%" cy="50%" outerRadius="80%" data={data}>
        ...
        <Tooltip contentStyle={{ backgroundColor: '#050505', borderColor: '#222', border: '1px solid rgba(255,255,255,0.1)' }} />
      </RadarChart>
    </ResponsiveContainer>
  </div>
));
...
export default function Analytics() {
  ...
  const volumeData = useMemo(() => progressData?.map(d => ({
    month: d.date,
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-4xl font-display font-bold tracking-widest uppercase text-glow">Neural Analytics</h1>
        <p className="text-muted-foreground font-sans mt-1">OPERATIVE PERFORMANCE VECTORS // CLASSIFIED DATA</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <motion.div 
          ...
          className="col-span-1 lg:col-span-2 bg-card/40 border border-white/5 rounded-2xl p-8 backdrop-blur-md"
        >
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-4">
               <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                  <div className="w-2 h-2 rounded-full bg-primary shadow-[0_0_5px_#39FF14]" /> Squat
               </div>
               <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                  <div className="w-2 h-2 rounded-full bg-accent shadow-[0_0_5px_#00E5FF]" /> Bench
               </div>
               <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                  <div className="w-2 h-2 rounded-full bg-purple-500 shadow-[0_0_5px_#A855F7]" /> Deadlift
               </div>
            </div>
          </div>
          <StrengthCurve data={progressData || []} />
        </motion.div>

        {/* Muscle Heatmap (Radar) */}
        <motion.div 
          initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }}
          className="bg-card/40 border border-white/5 rounded-2xl p-8 backdrop-blur-md"
        >
        ...
        >
           <h3 className="font-display font-bold text-xl uppercase tracking-widest mb-8 flex items-center gap-3">
             <Zap className="w-5 h-5 text-primary" /> 1RM Oracle Matrix
           </h3>
           <div className="space-y-4 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
             {prs?.map((pr, i) => (
               <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-secondary/10 border border-white/5 group hover:border-primary/20 transition-all">
                  <div>
                    <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{pr.name}</div>
                  ...
                  <div className="text-right">
                    <div className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">Estimated 1RM</div>
                    <div className="text-2xl font-display font-bold text-primary tracking-wider text-glow-small">
                      {Math.round(calculate1RM(pr.weight, 5))} <span className="text-[10px] text-primary/70 font-sans">LBS</span>
                    </div>
                  </div>
               </div>
             ))}
             {!prs?.length && <div className="text-center py-12 text-muted-foreground italic text-sm opacity-40 uppercase tracking-widest font-bold">Scanning for biometric data...</div>}
           </div>
        </motion.div>
